    ...
    if request.method == "POST":
        if request.POST.get("user_name") and request.POST.get("user_email") and request.POST.get("user_password") and request.POST.get("terms"):
            ...
            if Interpreter.objects.filter(username=info_dic["name"]).count() == 0 | Interpreter.objects.filter(email=info_dic["email"]).count() == 0:
                ...
                new_interpreter = requests.post(url=create_interpreter_endpoint, data={"username":str(info_dic["name"].lower()), "email":str(info_dic["email"]), "password":request.POST.get("user_password"), "is_active":True}, timeout=20)

                all_interpreters = requests.get(url=list_all_interpreters_endpoint, timeout=20)
